...
#define COMMANDGET      "getfaceurl"       //根据特征值获取人脸图片信息
...
#define REDISFACEURL    "face_url"  //海康云存人脸URL
#define REDISBKGURL     "bkg_url"   //海康云存背景URL
...
#define SQLMAXLEN       1024 * 8    //SQL语句最大长度
#define MAXIPLEN        64          //IP, FaceRect最大长度
#define MAXLEN          128          //FaceUUID, LibName, DeviceID最大长度
...
typedef struct _DeviceInfo
{
    ...
    int nLoadLimitTime;     //读取图片天数
    int nServerTime;        //是否以本地服务器时间为图片保存时间, 避免时间乱序带来的各种问题
    int nLoadURL;           //是否读取图片URL信息
...
typedef vector<LPSEARCHFEATURE> VECTORSEARCHFEATURE;
...
typedef struct _SearchResult
{
    ...
    char * pFeature;            //图片特征值
    char pFaceURL[2048];        //海康云存人脸URL
    char pBkgURL[2048];         //海康云存背景图URL
    _SearchResult()
    {
        memset(pFaceUUID, 0, sizeof(pFaceUUID));
...
typedef struct _SearchTask
{
    ...
    char pSearchTaskID[MAXLEN];     //检索任务ID
    unsigned int dwSearchTime;      //搜索开始时间, 用于判断搜索是否超时
    ...
    int nScore;                     //检索阈值
    VECTORSEARCHFEATURE vectorFeatureInfo;      //检索目标特征值信息(可以多张图片一起检索, 取并集, 即只要命中多个目标中的一个也返回结果)
    ...
    ~_SearchTask()
    {
        while (vectorFeatureInfo.size() > 0)
        {
            delete vectorFeatureInfo.back();
            vectorFeatureInfo.pop_back();
        }
...
//提供给kafka server4使用, 查询图片对应url信息
typedef struct _FaceURLInfo
{
    char pFaceUUID[MAXLEN];     //特征值FaceUUID
    int64_t nTime;              //抓拍时间
    char pDisk[2];              //盘符
