...
FeatureData *DataIterator::Next()
{
	...
	if (pdata && pdata->CollectTime <= m_endtime.collectTime && pdata->CollectTime >= m_begintime.collectTime)
	{
		return pdata;
	}
	return 0;
...
FeatureData *DataIterator::Previous()
{
    ...
    if (pdata && pdata->CollectTime >= m_begintime.collectTime && pdata->CollectTime <= m_endtime.collectTime)
    {
        return pdata;
    }
    return 0;
